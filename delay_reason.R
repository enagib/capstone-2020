...
on_time %>% 
  group_by(year) %>% 
  summarise(carrier = mean(carrier_avg_mins, na.rm = TRUE),
            weather = mean(weatheravg_mins, na.rm = TRUE),
            aviation = mean(national_aviation_systemavg_mins, na.rm = TRUE),
            late_arrival = mean(late_aircraft_arrivalavg_mins, na.rm = TRUE),
            security = mean(securityavg_mins, na.rm = TRUE)) %>% 
...
labs(lege)
  scale_fill_discrete(name = "Reason", labels = c("C", "W", "A", "L", "S")) +
  theme(legend.text = c("C", "W", "A", "L", "S"))
